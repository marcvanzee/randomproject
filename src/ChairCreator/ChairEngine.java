...
public class ChairEngine {
	...
	public void generateRandomPoints() {
		...
		for (int i = 0; i < points.length; i++) {
			points[i] = new Point3D(Calculator.randomDouble(DIMX), Calculator
					.randomDouble(DIMY), Calculator.randomDouble(DIMZ));
		}
	}
			...
			for (int b = (a + 1); b < points.length - 2; b++) {
				for (int c = (b + 1); c < points.length - 1; c++) {
					for (int d = (c + 1); d < points.length; d++) {
						...
						// eerst in zijn totaal controlleren of de random punten
						// tot een stoel kunnen komen.
						chair.build(a, b, c, d);
						if (chair.isValid())
							return chair;
					}
