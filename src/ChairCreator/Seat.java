    ...
    public void orden(Point3D[] square, Plane vectorSquare) {
        ...
        for (int i = 0; i < 4; i++){
            ...
            for (int j = 0; j < 4; j++){
                ...
                double direction = Calculator.pointToPlane(cut1, points[seatPoints[j]], false);
            ...
            if(min == plus) {
                ...
                if (direction < 0) seatCross = new int[]{minArray[0], minArray[1]};
                if (direction > 0) seatCross = new int[]{plusArray[0], plusArray[1]};
