...
public class Calculator extends ChairEngine {
    ...
    public static void transformation(Point3D transformPoints[], Point3D vectorPlaneSeat[]) {
        ...
        rValue = angle(new Point3D(vectorPlaneSeat[1].x, vectorPlaneSeat[1].y, 0), new Point3D(0, 0, 0), new Point3D(vectorPlaneSeat[1].x, 0, 0));
        rValue = rotationAngle(vectorPlaneSeat[1].x, vectorPlaneSeat[1].y, rValue);
        ...
        // nu moet vectorPlaneSeat1.z = 0 en de rest meeroteren en y buiten beschouwing gelaten worden
        rValue = angle(new Point3D(vectorPlaneSeat[1].x, 0, vectorPlaneSeat[1].z), new Point3D(0, 0, 0), new Point3D(vectorPlaneSeat[1].x, 0, 0));
        rValue = rotationAngle(vectorPlaneSeat[1].z, vectorPlaneSeat[1].x, rValue);
        ...
        // nu moet vectorPlaneSeat2.z = 0 en de rest meeroteren en x buiten beschouwing
        rValue = angle(new Point3D(0, vectorPlaneSeat[2].y, vectorPlaneSeat[2].z), new Point3D(0, 0, 0), new Point3D(0, vectorPlaneSeat[2].y, 0));
        rValue = rotationAngle(vectorPlaneSeat[2].z, vectorPlaneSeat[2].y, rValue);
        ...
        center.x = (vectorPlaneSeat[0].x + vectorPlaneSeat[1].x + vectorPlaneSeat[2].x + vectorPlaneSeat[3].x) / 4;
        center.y = (vectorPlaneSeat[0].y + vectorPlaneSeat[1].y + vectorPlaneSeat[2].y + vectorPlaneSeat[3].y) / 4;
        center.z = (vectorPlaneSeat[0].z + vectorPlaneSeat[1].z + vectorPlaneSeat[2].z + vectorPlaneSeat[3].z) / 4;

        translation(transformPoints, center);
        rotation(transformPoints, 45, AXES_XY);
    }

    public static void translation(Point3D[] translatePoints, Point3D translation) {
        double x, y, z;
    ...
    public static void rotation(Point3D[] rotationPoints, double rValue, int Axes) {
        ...
        switch (Axes) {
            case 0: // front view
                ...
                for (int i = 0; i < rotationPoints.length; i++) {
                    ...
                    rotationPoints[i].x = (double) (x * Math.cos(rValue) - y * Math.sin(rValue));
                    rotationPoints[i].y = (double) (x * Math.sin(rValue) + y * Math.cos(rValue));
                }
                break;
            case 1: // top view
                ...
                for (int i = 0; i < rotationPoints.length; i++) {
                    ...
                    rotationPoints[i].x = (double) (x * Math.cos(rValue) - z * Math.sin(rValue));
                    rotationPoints[i].z = (double) (x * Math.sin(rValue) + z * Math.cos(rValue));
                }
                break;
            case 2: // side view
                ...
                for (int i = 0; i < rotationPoints.length; i++) {
                    double y = rotationPoints[i].y;
                    double z = rotationPoints[i].z;
                    rotationPoints[i].y = (double) (y * Math.cos(rValue) - z * Math.sin(rValue));
                    rotationPoints[i].z = (double) (y * Math.sin(rValue) + z * Math.cos(rValue));
                }
                break;
        }
    }
    ...
    public static Point3D[] shortestSquare(Point3D[] points1) {
        ...
        double test1 = lineLength(points1[0], points1[1]) + lineLength(points1[1], points1[2]) + lineLength(points1[2], points1[3]) + lineLength(points1[3], points1[0]);
        double test2 = lineLength(points1[0], points1[1]) + lineLength(points1[1], points1[3]) + lineLength(points1[2], points1[3]) + lineLength(points1[2], points1[0]);
        double test3 = lineLength(points1[0], points1[3]) + lineLength(points1[1], points1[3]) + lineLength(points1[1], points1[2]) + lineLength(points1[2], points1[0]);
    ...
    public static int[] shortestSquare(int[] pointers) {
        ...
        Point3D tempPoint = new Point3D();
        int tempPointer;
        Point3D[] points1 = new Point3D[]{points[pointers[0]], points[pointers[1]], points[pointers[2]], points[pointers[3]]};
    ...
    public static Vector perpVector(Vector vec1, Vector vec2, double num) {
        ...
        Vector vector = new Vector(normalVector.dX, normalVector.dY, normalVector.dZ);
    ...
    public static Vector perpVector(Point3D point1, Point3D point2, Point3D point3, double num) {
    ...
    public static Point3D offsetPoint(Point3D p1, Point3D p2, Point3D p3, double length) {

        Vector leadVector = getLeadVector(p1, p2, p3);

        //DIKTE GEDEELD DOOR SCHUINE ZIJDE OM VERMENIGVULDIGINGSFACTOR TE BEPALEN
        double fac = length / leadVector.length();

        //VERPLAATSING DELTA X EN Y BEPALEN AAN DE HAND VAN FAC. EN DELTA LOODLIJN
    ...
    public static Point3D intersectPoint(Point3D point1, Vector vector1, Point3D point2, Vector vector2) {

        //a (V1 X V2) = (P2 - P1) X V2

        Point3D point = new Point3D((point2.x - point1.x), (point2.y - point1.y), (point2.z - point1.z));
        ...
        point = new Point3D((point1.x - point2.x), (point1.y - point2.y), (point1.z - point2.z));
        ...
        Point3D intersectPoint1 = new Point3D((point1.x + (a * vector1.dX)), (point1.y + (a * vector1.dY)), (point1.z + (a * vector1.dZ)));
        Point3D intersectPoint2 = new Point3D((point2.x + (b * vector2.dX)), (point2.y + (b * vector2.dY)), (point2.z + (b * vector2.dZ)));
    ...
    public static Point3D intersectPoint(Point3D point1, Vector vec1, Plane plane1) {

        double top = ((plane1.a * point1.x) + (plane1.b * point1.y) + (plane1.c * point1.z) + plane1.d);
    ...
    public static Vector correctPerpVector(Edge shared, Edge edge1, Edge edge2, Vector v1, Vector v2) {
        int p1 = (edge1.a == shared.a || edge1.a == shared.b) ? edge1.b : edge1.a;
        int p2 = (edge2.a == shared.a || edge2.a == shared.b) ? edge2.b : edge2.a;

        return correctPerpVector(shared, p1, p2, v1, v2);
    }

    public static Vector correctPerpVector(Edge edge1, int p1, int p2, Vector perp1, Vector perp2) {
        ...
        Point3D intersVecOnVec = intersectPoint(tempPoint1, vec2, tempPoint2, vec1);
    ...
    public static Point3D getLeadPoint(Point3D p1, Point3D p2, Point3D p3) {
        ...
        double leadDist = (lineLength(p2, p3) * Math.cos(Math.toRadians(angle(p1, p2, p3))));

        //ORIGINELE DELTA X EN Y BEPALEN VAN ZIJDE DRIEHOEK.
        Vector original = new Vector(p2, p1);

        // gebruik de verhouding p2-loodpunt / p2-p1 om de delta x en y en z van het loodpunt te weten te komen
    ...
    public static Vector getLeadVector(Vector vec1, Vector vec2) {
        ...
        return getLeadVector(new Point3D(0, 0, 0), new Point3D(vec1.dX, vec1.dY, vec1.dZ), new Point3D(vec2.dX, vec2.dY, vec2.dZ));
    }
    ...
    public static double pointToPlane(Plane plane, Point3D point, boolean abs) {
        ...
        if (abs == true) {
            x = Math.abs((plane.a * point.x) + (plane.b * point.y) + (plane.c * point.z) + plane.d);
        } else {
            x = (plane.a * point.x) + (plane.b * point.y) + (plane.c * point.z) + plane.d;
        }
        double y = Math.sqrt((plane.a * plane.a) + (plane.b * plane.b) + (plane.c * plane.c));
    ...
    public static double pyramideVolume(Point3D point1, Point3D point2, Point3D point3, Point3D point4) {
    ...
    public static double vecAngle(Vector v1, Vector v2) {
        ...
        return angle(new Point3D(v1), new Point3D(0.0, 0.0, 0.0), new Point3D(v2));
    ...
    public static double rotationAngle(double pointA, double pointB, double rValue) {
        if (pointA > 0) {
            if (pointB > 0) {
                rValue = 360 - rValue;
            }
    ...
    public static int[][] permutation(int offset, int[] input, int[][] output, int length) {  //by Jesper Nordenberg addapted by OTN
    ...
    public static boolean onSquare(Point3D[] square1, Point3D point1) {    //niet doorgetest, dus niet zeker was van zijn werken.

        LOOP1:
        for (int i = 0; i < 2; i++) {

            Point3D interPoint = intersectPoint(square1[i], new Vector(square1[i], square1[(i + 1) % 4]),
                    square1[(i + 2) % 4], new Vector(square1[(i + 2) % 4], square1[(i + 3) % 4]));


            if (interPoint != null && onLine(square1[i], square1[(i + 1) % 4], interPoint)) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                Point3D interPoint = intersectPoint(point1, vec1, square1[(j)], new Vector(square1[j], square1[(j + 1) % 4]));

                if (interPoint != null && onLine(square1[j], square1[(j + 1) % 4], interPoint)) {
                    if (tempPoint.x == 0) {
