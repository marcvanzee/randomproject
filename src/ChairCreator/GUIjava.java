...
public class GUIjava extends Applet implements  MouseListener,
    ...
    void createChair(){
        ...
        //for(int i = 0; i < tryChair.messages.length; i++)System.out.println(tryChair.messages[i]);
    ...
    void sortEdges(){
        ...
        //TEKEN DE TE TEKENEN LIJNEN VAN DE GEGEVEN DRIEHOEKEN, ZONDER LIJNEN TWEE KEER TE TEKENEN
	...
	void drawWireframe( Graphics g ) {
            ...
            //defining the planes of the back and the seat, jezus kanker stijve hark.

            for (int i = 0; i < triangles.length; ++i ) {

                g.setColor( Color.LIGHT_GRAY );

                g.fillPolygon(  new int[]{points[ triangles[i].point[0] ].x, points[ triangles[i].point[1] ].x, points[ triangles[i].point[2] ].x},
                                new int[]{points[ triangles[i].point[0] ].y, points[ triangles[i].point[1] ].y, points[ triangles[i].point[2] ].y}, 3);
            ...
            for (int j = 0; j < triangleEdges.length; ++j ) {
                g.drawLine( points[ triangleEdges[j].a ].x, points[ triangleEdges[j].a ].y,
                            points[ triangleEdges[j].b ].x, points[ triangleEdges[j].b ].y );
	...
	void realPointsToPoints(Point3D[] realPoints, Point[] points) {
            for (int i = 0; i < vertices.length; ++i ) {
                ...
                points[i] = new Point( (int) realPoints[i].x,
                                       (int) realPoints[i].y );
            }
	}

        void sort (Point3D[] realPoints, int[] positions){		// gaat er problematisch genoeg nu van uit dat je sowieso werkt met 3 hoeken
            double[] totalDepths = new double[triangles.length];	// maar in alle eerlijkheid zijn vierkanten op dit moment toekomst muziek.

            for (int i = 0; i < triangles.length; ++i) {
                double totalDepth = realPoints[triangles[i].point[0]].z + realPoints[triangles[i].point[1]].z + realPoints[triangles[i].point[2]].z;
    ...
    public final void getContents(String[] newValue) throws FileNotFoundException {
        ...
        try {
            ...
            while ( scanner.hasNextLine() ){
                ...
                if (newValue == null) settings(currentContents);
                else {
                    if (i == 0){
                        if (currentContents[0].equals(newValue[0])) newContents.concat( (newValue[0] + " = " + newValue[1]) );
                        else                                        newContents.concat( (currentContents[0] + " = " +currentContents[1]) );


                System.out.println(newContents);

                    } else {
                        if (currentContents[0].equals(newValue[0])) newContents.concat( ("\n" + newValue[0] + " = " +newValue[1]) );
                        else                                        newContents.concat( ("\n" + currentContents[0] + " = " +currentContents[1]) );
    ...
    public void settings(String[] strings){
        ...
        for (int i =0; i< strings.length; i++){
            ...
            if (setting.equals("SEAT_STABILITY")) tryChair.SEAT_STABILITY = Double.parseDouble(strings[1]);
            else if (setting.equals("SEAT_STEEP")) tryChair.SEAT_STEEP = Double.parseDouble(strings[1]);
            else if (setting.equals("SEAT_MIN_HEIGHT")) tryChair.SEAT_MIN_HEIGHT = Double.parseDouble(strings[1]);
            else if (setting.equals("SEAT_MAX_HEIGHT")) tryChair.SEAT_MAX_HEIGHT = Double.parseDouble(strings[1]);
            else if (setting.equals("BACK_MIN_ANGLE")) tryChair.BACK_MIN_ANGLE = Double.parseDouble(strings[1]);
            else if (setting.equals("BACK_MAX_ANGLE")) tryChair.BACK_MAX_ANGLE = Double.parseDouble(strings[1]);
            else if (setting.equals("LEGS_STABILITY")) tryChair.LEGS_STABILITY = Double.parseDouble(strings[1]);
            else if (setting.equals("CHAIR_HEIGHT")) tryChair.CHAIR_HEIGHT = Double.parseDouble(strings[1]);
